...
4. **Seed problems.** What recurring tensions does the user already know? Capture 5–15 raw problem statements in their words. Each candidate must pass the forces test (a tension, not an absence) before becoming a pattern — help reformulate the ones that fail. When Q6 surfaces real material, prefer running the mining pipeline (`references/mining.md`) over collecting seed problems by recall — mined candidates arrive with proto-known-uses attached.
