...
- **Evidence & provenance** — where the patterns came from: what material was mined (retros, decision logs, interviews, fieldwork), when, and by whom. Note the overall confidence honestly — a language born entirely at confidence 0 is fine, but say so here. **Declare the evidence semantics**: state explicitly what counts as a known-use *instance* for this language (see `pattern-form.md` rule 6 and Confidence calibration). For most languages the field is real practice; for a play-facing game language it is the table (documented campaigns and sessions, not genre convention); for a fiction-authoring language whose deliverable is coherent fiction, specific named works may count. This declaration is what a validator's qualitative pass judges the asterisks against — evidence semantics improvised per-pattern instead of declared here is a HIGH finding.
