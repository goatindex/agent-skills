...
6. **Known uses** (optional section, after Therefore): one list item per real, independent **instance** of *this* pattern working — the PLoP tradition's evidence slot. This is where confidence is *earned*: the validator warns on a ✻✻ pattern with no Known uses section. Evidence comes in three tiers, and **only the first belongs here**:
   - **Instance** — a specific, nameable, dateable occasion on which this exact pattern was applied *in the language's declared application domain* (the pathfinder's Purpose section defines that domain) and the outcome observed: "the 2024 restructure group kept the trail and the merger held." The test: an instance can be named and dated; if it can only be described in vague plurals ("teams consistently…", "recurring across…"), it is not an instance. Instances promote confidence.
   - **Designed-system corroboration** — an independent system that *encodes* the same resolution ("Blades in the Dark's engagement roll rewards scouting before a score", "CODEOWNERS files encode the same stewardship"). Strong evidence the tension and solution-shape are real; not an instance of this pattern in this context. Belongs in body prose, never in Known uses; never promotes alone.
   - **Narrative convention** — recurrence across a fiction corpus ("crews that skip legwork are consistently shown failing"). Evidence the *tension resonates*, and zero evidence the *solution works* — fiction is a biased simulator whose outcomes are chosen by authors for drama. Belongs in the Problem body's argument if anywhere; never promotes.

   If you have no genuine instances, leave the section out and stay at confidence 0 — an empty-but-honest known-uses record beats a padded one, and disclosing that corroboration was promoted anyway does not make the promotion legitimate.
...
Calibrate against **known uses** — meaning *instances* in the sense of rule 6, never corroboration or convention (the pattern community's "rule of three": one instance is an event, two a coincidence, three a pattern):

- `0` (no asterisk) — a hypothesis worth recording. You believe it; you have not tested it. No known uses required.
- `1` (✻) — real progress on the problem; at least **one** genuine instance, but better formulations surely exist.
- `2` (✻✻) — you would be surprised to see a living solution to this problem that does not contain this pattern in some form. Rare. Demands roughly **three independent instances** documented in the `## Known uses` section, plus evidence in the body.

Corroboration and convention strengthen the body's argument at any confidence level, but **never promote by themselves** — there is no density of genre convention that adds an asterisk. What counts as an instance is *relative to the language's declared purpose*, and the pathfinder's Evidence section must say which semantics apply: for a play-facing game language the field is the table, so instances are documented campaigns and sessions; for a fiction-authoring language whose deliverable *is* coherent fiction, a specific named work where the convention demonstrably holds can count as an instance — but it must still be specific and nameable, not "the genre generally does this."
