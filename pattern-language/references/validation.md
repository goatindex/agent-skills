...
6. **Gradient coherence.** Do the scale groups descend a real, declared, monotone axis (spatial scale, scope, process order — see `language-structure.md` § The gradient), or are they unordered categories? Categories with no ordering rationale are the smell — the reader should descend. Two things to verify against `PATHFINDER.md` § The gradient: **(a) single axis, no drift** — the bands must descend *one* axis the whole way, not start on scope and slide into chronology partway down; **(b) label matches structure** — the axis the pathfinder *names* must be the axis the bands actually descend. A language whose bands form a clean formation-chronology (understand → form → equip → execute) but whose pathfinder calls it "organizational scope" has mislabeled its own gradient; either fix the label or re-order to the named axis. (Both are common weak-model errors — the model reaches for an intuitive process ordering, then mislabels it with the doctrine's headline term "scale/scope".)
